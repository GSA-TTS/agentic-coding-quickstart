...
#
#   acq_prompt_multiselect DEFAULTS_CSV LABEL_1 DESC_1 [LABEL_2 DESC_2 ...]
#       Present a checkbox list of items. DEFAULTS_CSV is a comma-separated list
#       of 1-based indices pre-checked on entry (empty = none). Interactive: the
#       user moves with the arrow keys (or j/k), toggles with SPACE, confirms with
#       ENTER, aborts with q/ESC. On confirm, echoes the CHOSEN 1-based indices as
#       a space-separated list on STDOUT (empty line = nothing chosen). All chrome
#       is written to STDERR so a caller can capture the selection cleanly.
#
...
# acq_prompt_multiselect DEFAULTS_CSV LABEL DESC [LABEL DESC ...]
# See the header for the full contract. The chosen 1-based indices are placed in
# the global _ACQ_PROMPT_SELECTION (space-separated; "CANCELLED" on abort) AND
# echoed on stdout. Prefer the GLOBAL when composing widgets in one process (e.g.
# acq_configure): a `$(...)` capture forks a subshell, which would discard the
# shared scripted-input cursor advance and desync a following acq_prompt_confirm.
_ACQ_PROMPT_SELECTION=""
acq_prompt_multiselect() {
  _ACQ_PROMPT_SELECTION=""
  local defaults_csv="${1:-}"; shift || true

  # Collect items into parallel index/label/desc arrays (bash 3.2 safe).
  ...
  # Seed the checked set from DEFAULTS_CSV (comma-separated 1-based indices).
  local checked="" idx
  ...
  for idx in "$@"; do
    case "$idx" in
      ...
      *) [ "$idx" -ge 1 ] && [ "$idx" -le "$n" ] && checked="$checked $idx" ;;
    esac
  done

  # Non-interactive: emit the defaults verbatim and return without any chrome.
  if ! _acq_prompt_interactive; then
    _ACQ_PROMPT_SELECTION="$(printf '%s' "$checked" | sed 's/^ *//')"
    printf '%s\n' "$_ACQ_PROMPT_SELECTION"
    return 0
  fi

  _acq_prompt_set_colors
  local cursor=1 key i mark row line
  ...
  while :; do
    ...
    if [ "$rendered" -gt 0 ]; then
      ...
      printf '%s%s?%s %sSelect kits%s %s(↑/↓ move · SPACE toggle · ENTER confirm · q cancel)%s\n' \
    ...
    key="$_ACQ_PROMPT_KEY"
    case "$key" in
      UP)    cursor=$((cursor - 1)); [ "$cursor" -lt 1 ] && cursor="$n" ;;
      DOWN)  cursor=$((cursor + 1)); [ "$cursor" -gt "$n" ] && cursor=1 ;;
      SPACE)
        if _acq_prompt_in_list "$cursor" $checked; then
          # Remove cursor from the checked set.
          local new="" c
  ...
  # Emit the chosen indices in ascending order on stdout AND in the global.
  local out="" j
  j=1
  while [ "$j" -le "$n" ]; do
    _acq_prompt_in_list "$j" $checked && out="$out $j"
    j=$((j + 1))
  done
  _ACQ_PROMPT_SELECTION="$(printf '%s' "$out" | sed 's/^ *//')"
  printf '%s\n' "$_ACQ_PROMPT_SELECTION"
  return 0
}

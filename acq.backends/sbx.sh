...
acq_backend_secret_set() {
  ...
  if [ -z "$service" ]; then
    ...
    echo "     usage: acq secret set <service> [--host HOST --env ENV] [--force]" >&2
    exit 1
  fi
  ...
  if [ -n "$host" ] || [ -n "$env_var" ]; then
    # Custom form. Build the command without secret in argv.
    ...
    # Prompt the user for the key; pipe via stdin to sbx (never argv).
    ...
    case "$_ACQ_SBX_BUILTIN_SERVICES" in
      *" $service "*)
        sbx secret set -g "$service" "${extra_flags[@]+"${extra_flags[@]}"}"
        ;;
      *)
        echo "acq: '$service' is not a known built-in sbx service." >&2
        echo "     For custom endpoints use: acq secret set $service --host HOST --env ENV" >&2
        exit 1
        ;;
    esac
  fi

...
| `startup` | configurable (default agent uid) | on acq resume (see note), after initFiles | `commands.startup` | `msb exec` on `acq run`/`acq start`/`acq restart` (acq re-drives the idempotent apply); a raw `msb start` outside acq replays it only if the experimental `--entrypoint` persistence is enabled (ADR-0017) | every-boot part of `provision.sh` after initFiles block |

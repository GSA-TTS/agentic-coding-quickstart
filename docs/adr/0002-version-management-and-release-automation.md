...
3. **release-please for Release Automation**
   ...
   - Release creation also uploads installer assets: `install.sh` with the release
     ...
     downloaded installer assets
   - Release automation publishes GitHub artifact attestations for `install.sh`,
     `install.ps1`, the Windows preview zip, and `SHA256SUMS`, tying those assets
     to the GitHub Actions workflow identity that built them
...
7. Upload release installer assets (`install.sh`, `install.ps1`, the Windows
   preview zip, and `SHA256SUMS`) when a release is created
8. Publish GitHub artifact attestations for the release assets
